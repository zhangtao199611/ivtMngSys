...
<div class="admin-content-body">
    <div class="am-cf am-padding am-padding-bottom-0">
        <c:if test="${status.stockDetailType==0}">
            <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">出库明细</strong><small></small></div>
        </c:if>
        <c:if test="${status.stockDetailType==1}">
            <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">入库明细</strong><small></small></div>
        </c:if>
        <c:if test="${status.stockDetailType==2}">
            <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">损耗明细</strong><small></small></div>
        </c:if>
        <c:if test="${status.stockDetailType==3}">
            <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">退货明细</strong><small></small></div>
        </c:if>
    </div>
            ...
            <form class="am-form">
                <table class="am-table am-table-striped am-table-hover table-main">
                    ...
                    <tr>
                        ...
                        <c:if test="${status.stockDetailType==0}">
                            <th class="table-title">出库量</th>
                        </c:if>
                        <c:if test="${status.stockDetailType==1}">
                            <th class="table-title">入库量</th>
                        </c:if>
                        <c:if test="${status.stockDetailType==3}">
                            <th class="table-title">退货量</th>
                        </c:if>
                        <c:if test="${status.stockDetailType==3}">
                            <th class="table-title">退货理由</th>
                        </c:if>
                    ...
                    <c:forEach items="${users}" var="u" varStatus="cou">
                        <tr>
                            ...
                            <td style="text-align: center;display: table-cell;vertical-align: middle" class="first"><span style="color: black;font-weight: bold">${u.theProduct} </span><a href="${ctx}/ProductController/toProductDetail?id=${u.stockDetailID}&currentPage=${page.currentPage}">${u.stockID}</a></td>
                            <c:if test="${status.stockDetailType!=3}">
                                <td style="display:table-cell; vertical-align:middle" class="weighing">
                                    ...
                                    <c:if test="${u.weighingWeight!='0.000'}">
                                    ${u.weighingWeight} <font style="font-weight: initial;color: black">${u.unit}</font>
                                    </c:if>
                                </td>
                            </c:if>
                            <c:if test="${status.stockDetailType!=3}">
                            ...
                            <td style="display:table-cell; vertical-align:middle" class="bePutInStorage">
                                ...
                                <c:if test="${u.integer==null}">
                                    ${u.weight}
                                        ...
                                        <font style="font-weight: initial;color: black">${u.unit}</font> </td>
                            <c:if test="${status.stockDetailType==3}">
                                <td style="color: red;width: 13%;display:table-cell; vertical-align:middle;font-weight: bold">
                                    <c:if test="${u.remarks==''}">
                                        <span style="font-weight: initial;color: black">/</span>
                                    </c:if>
                                    <c:if test="${u.remarks!=null}">
                                        ${u.remarks}
                                    </c:if>
                            ...
                            <td style="width: 12%;display:table-cell; vertical-align:middle">
                                <div class="am-btn-group am-btn-group-xs">
                                    <c:forEach items="${permissions}" var="p" varStatus="s">
                                        <c:if test="${s.count=='60'}">
                                            <c:if test="${p=='1'}">
                                                <button class="am-btn am-btn-default am-btn-xs am-text-secondary" type="button" onclick="javascipt:window.location.href='${ctx}/StockDetailInfoController/findStockDetail?stockDetailID=${u.stockDetailID}&currentPage=${page.currentPage}&userName=${userName}'"><span class="icon-list"></span>明细详情</button>
                                            </c:if>
                                        </c:if>
                                    </c:forEach>

                                    <c:forEach items="${permissions}" var="p" varStatus="s">
                                        <c:if test="${s.count=='61'}">
                                            <c:if test="${p=='1'}">
                                                <button class="am-btn am-btn-default am-btn-xs am-text-secondary" type="button" onclick="javascipt:window.location.href='${ctx}/StockDetailInfoController/updatePageStockDetail?stockDetailID=${u.stockDetailID}&currentPage=${page.currentPage}&userName=${userName}'"><span class="icon-pencil"></span>修改明细</button>
                                            </c:if>
                                        </c:if>
                                    </c:forEach>
                                </div>
