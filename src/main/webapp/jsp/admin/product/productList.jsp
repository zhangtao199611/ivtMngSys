        ...
        <div class="am-u-sm-12">
            <form class="am-form">
                <table class="am-table am-table-striped am-table-hover table-main">
                    <thead>
                    <tr>
                        <%--                <!--批量修改价格暂时没做完，先不放出来-->           <th class="table-title">价格修改</th>--%>
                    ...
                    <tr v-for="(d,i) in list" :title="d.description" :class="d.status==1?'gray_text':''">
                        <%--             <!--批量修改价格暂时没做完，先不放出来-->              <td style="display:table-cell; vertical-align:middle"><input type="checkbox"--%>
                        <%--                                                                                     :checked='d.ischecked'--%>
                ...
                <div class="selected_prod">
                    <%--   <!--批量修改价格暂时没做完，先不放出来-->                 <h2>修改价格产品：</h2>--%>
    ...
    new Vue({
        ...
        methods: {
            ...
            submitUpdatePriceProdList() {
                ...
                this.$http.post('${ctx}/ProductController/setUpdatPriceProdList', JSON.stringify(this.updatePriceProdList), {
                    headers: {
                        "Content-Type": "application/json;charset=utf-8"  //头部信息
                    }
                })
                    .then(function (response) {
                        alert((response.data.result == 1 ? "修改成功，" : "添加失败，") + (response.data.msg != null ? response.data.msg : ""));
                        if (response.data.result == 1) {
                            window.location.href = "${ctx}/ProductController/toProductList";
                        }
                    })
