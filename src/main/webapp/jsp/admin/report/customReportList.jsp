        ...
        <div class="am-u-sm-12 am-u-md-6">
            <div class="am-btn-toolbar">
                <div class="am-btn-group am-btn-group-xs">
                    <span style="font-weight: bold;color: #0e90d2">选择时间：开始日期：<input type="date" id="start"
                                                                                    value="${start}"/>  结束日期：<input
                            type="date" id="end" value="${end}"/><button
                            onclick="customDateTimeOnclick()">确定</button></span>
                        ...
                        <ul class="am-pagination">
                            <c:if test="${page.currentPage==1}">
                                <li class="am-disabled"><a
                                        href="${ctx}/reportController/customReportListUpDown?pageSize=${page.pageSize}&currentPage=${page.currentPage-1}&starts=${start}&ends=${end}">«</a>
                                </li>
                            </c:if>
                            <c:if test="${page.currentPage>1}">
                                <li>
                                    <a href="${ctx}/reportController/customReportListUpDown?pageSize=${page.pageSize}&currentPage=${page.currentPage-1}&starts=${start}&ends=${end}">«</a>
                                </li>
                            </c:if>
...
<script type="text/javascript">
    ...
    function customDateTimeOnclick() {
        if ($("#start").val() == '' || $("#end").val() == '' || $("#end").val() == null || $("#start").val() == null) {
            alert("请输入日期");
        } else {
            window.location.href = "${ctx}/reportController/customReportList?start=" + $("#start").val() + "&end=" + $("#end").val();
        }
    }
