...
<div class="admin-content-body" id="app">
	<div class="am-cf am-padding am-padding-bottom-0">
		<div class="am-fl am-cf">
			<strong class="am-text-primary am-text-lg">菜品类别管理</strong><small></small>
		</div>
	</div>
	<hr>
		...
		<div class="am-u-sm-12">
			<form class="am-form">
				<table class="am-table am-table-striped am-table-hover table-main">
					<thead>
					<tr>
						<th class="table-title">菜品类别名称</th>
					...
					<tr v-for="(d,i) in list" :class="d.statuss==1?'gray_text':''">
						<td>{{d.className}}</td>
						<td>
							<!--up-->
						...
						<td>
							<div class="am-btn-group am-btn-group-xs">
								<button type="button" v-if="d.statuss == 0"
										@click.prevent="updateClass(d.classID,d.className)"
										class="am-btn am-btn-default am-btn-xs am-text-secondary">
									<span class="am-icon-pencil-square-o"></span> 编辑
								</button>

								<button v-if="d.statuss == 0" type="button"
								...
								<button v-if="d.statuss == 1" type="button"
...
<script type="text/javascript">
	...
	new Vue({
		...
		mounted() {
			this.$http.get('${ctx}/DisClassInfoController/getClassList?currentPage= 1').then(response => {
		...
		methods: {
			...
			delClass(id, name) {
				if (confirm("确定删除“" + name + "”吗？删除后所有“" + name + "” 的菜品将不可见。")) {
					this.$http.get('${ctx}/DisClassInfoController/delClass?id=' + id + "&currentPage= " + this.page.currentPage).then(
							response => {
								this.list = response.data.resultData;
								this.page = response.data.pageModel;
			...
			//新增大类
			addClass() {
				var className = prompt("请输入菜品类别名称", "");
				if (className != "" && className != null) {
					if (confirm("确定添加吗？")) {
						this.$http.get('${ctx}/DisClassInfoController/addClassInfo?className=' + className + "&currentPage= " + this.page.currentPage).then(
								response => {
									this.list = response.data.resultData;
									this.page = response.data.pageModel;
			...
			recoverClass(id, name) {
				if (confirm("确定恢复“" + name + "”吗？此类别将在安卓平板上不可见。")) {
					this.$http.get('${ctx}/DisClassInfoController/recoverClass?id=' + id + "&currentPage= " + this.page.currentPage).then(
							response => {
								this.list = response.data.resultData;
								this.page = response.data.pageModel;
			...
			showClass(id, name) {
				if (confirm("确定展示“" + name + "”吗？此类别将在安卓平板上可见。")) {
					this.$http.get('${ctx}/DisClassInfoController/showClass?id=' + id + "&currentPage= " + this.page.currentPage).then(
							response => {
								this.list = response.data.resultData;
								this.page = response.data.pageModel;
			...
			hideClass(id, name) {
				if (confirm("确定隐藏“" + name + "”吗？此类别将在安卓平板上不可见。")) {
					this.$http.get('${ctx}/DisClassInfoController/hideClass?id=' + id + "&currentPage= " + this.page.currentPage).then(
							response => {
								this.list = response.data.resultData;
								this.page = response.data.pageModel;
			...
			//修改大类
			updateClass(id, name) {
				var newName = prompt("请输入新的菜品类别名字", name);
				if (newName != "" && newName != null) {
					if (confirm("确定将“" + name + "”修改为“" + newName + "”吗？")) {
						this.$http.get('${ctx}/DisClassInfoController/updateClassInfo?id=' + id + '&newName=' + newName + "&currentPage= " + this.page.currentPage).then(
								response => {
									this.list = response.data.resultData;
									this.page = response.data.pageModel;
			...
			listByPage(currentPage) {
				this.$http.get('${ctx}/DisClassInfoController/getClassList?currentPage= ' + currentPage).then(response => {
					console.log(response.data);
					this.list = response.data.resultData;
					this.page = response.data.pageModel;
				});
			...
			//上调排序
			upSortClass(id) {
				this.$http.get('${ctx}/DisClassInfoController/sortClass?id=' + id + '&upOrDown=' + 1 + '&currentPage= ' + this.page.currentPage).then(response => {
					console.log(response.data);
					this.list = response.data.resultData;
					this.page = response.data.pageModel;
			...
			//下调排序
			downSortClass(id) {
				this.$http.get('${ctx}/DisClassInfoController/sortClass?id=' + id + '&upOrDown=' + 0 + '&currentPage= ' + this.page.currentPage).then(response => {
					console.log(response.data);
					this.list = response.data.resultData;
					this.page = response.data.pageModel;
					alert((response.data.result == 1 ? "排序成功，" : "排序失败，") + (response.data.msg != null ? response.data.msg : ""));
