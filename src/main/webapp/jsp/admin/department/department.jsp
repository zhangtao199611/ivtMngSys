					...
					<div class="am-btn-toolbar">
						<div class="am-btn-group am-btn-group-xs">
							<%--<c:forEach items="${permissions}" var="p" varStatus="s">
								<c:if test="${s.count=='52'}">
									<c:if test="${p=='1'}">
									</c:if>
								</c:if>
							</c:forEach>--%>
							<a class="am-btn am-btn-default" href="${ctx}/UserInfoController/jumpAddEpartment">  <span class="am-icon-plus"></span>新增 </a>
...
<%--										<input type="checkbox" id="checkAll"onclick="setAllNo()">--%>
									...
									<th class="table-title">部门说明</th>
									<th class="table-type">部门成立时间</th>
									<th class="table-type">部门状态</th>
									<th class="table-set">操作</th>
								</tr>
							</thead>
							<tbody>
							   <c:forEach items="${deptInfos}" var="u">    
								<tr>
									<td>
<%--										<input type="checkbox" name="checkb" class="checkb">--%>
									</td>
									<td><a href="${ctx}/UserInfoController/findDepartment?deptID=${u.deptID}&currentPage=${page.currentPage}">D${u.deptID}</a></td>
									<td>${u.deptName}</td>
									<td>${u.deptExplain}</td>
									<td>${u.establishedStr}</td>
									<c:if test="${u.deptStatus==0}">
										<td style="color: #00aa00;font-size: 15px;font-weight: bold">正常</td>
									</c:if>
									<c:if test="${u.deptStatus==1}">
									...
									<c:if test="${u.deptStatus==0}">
										<td>
											<div class="am-btn-group am-btn-group-xs">
<%--												<c:forEach items="${permissions}" var="p" varStatus="s">--%>
<%--													<c:if test="${s.count=='53'}">--%>
<%--														<c:if test="${p=='1'}">--%>
<%--														</c:if>--%>
<%--													</c:if>--%>
<%--												</c:forEach>--%>
												<button class="am-btn am-btn-default am-btn-xs am-text-secondary" type="button" onclick="javascipt:window.location.href='${ctx}/UserInfoController/jumpUpdateDepartment?deptID=${u.deptID}&currentPage=${page.currentPage}'"><span class="am-icon-pencil-square-o"></span> 编辑 </button>

												<%--<c:forEach items="${permissions}" var="p" varStatus="s">
													<c:if test="${s.count=='54'}">
														<c:if test="${p=='1'}">
														</c:if>
													</c:if>
												</c:forEach>--%>
	<button class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only" type="button" onclick="javascript:if(confirm('确定冻结?'))location='${ctx}/UserInfoController/deleteDepartment?deptID=${u.deptID}&currentPage=${page.currentPage}'">  <span class="am-icon-trash-o"></span>冻结 </button>
											</div>
										</td>
									</c:if>
									...
									<c:if test="${u.deptStatus==1}">
										<td>
											<div class="am-btn-group am-btn-group-xs">
												<%--<c:forEach items="${permissions}" var="p" varStatus="s">
													<c:if test="${s.count=='55'}">
														<c:if test="${p=='1'}">
														</c:if>
													</c:if>
												</c:forEach>--%>
												<button class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only" type="button" onclick="javascript:if(confirm('确定解冻?'))location='${ctx}/UserInfoController/defrostDepartment?deptID=${u.deptID}&currentPage=${page.currentPage}'">  <span class="am-icon-trash-o"></span>解冻 </button>
											</div>
										</td>
									</c:if>
